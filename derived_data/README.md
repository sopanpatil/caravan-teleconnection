...
| `response_observed_DJF.parquet` | 2135 | 17 | All three properties recomputed on observed discharge, plus the gap-masked simulated memory used for the like-for-like comparison |
...
| `physiographic_coeffs_{full,independent}.csv` | 60 | 7 | Standardised coefficients, country-clustered SEs, wild cluster bootstrap p and BH q, per response and predictor (SI Table S2) |
| `physiographic_summary_{full,independent}.csv` | 10 | 23 | Marginal and within-country R2, ICC, and cross-validation skill under both references, per response (SI Table S1) |
| `physiographic_by_country_{full,independent}.csv` | 157 | 11 | Leave-one-country-out skill per withheld country, decomposed into level offset and within-country ordering (SI Table S3) |
...
Reproducible here, and checked to match the archive exactly: the
precipitation signal (`fit_precipitation_signal.py`), the spatial-independence
screen (`nesting_screen.py`), the whole physiographic synthesis and both
cross-validation schemes (`physiographic_synthesis.py`, SI Tables S1 to S3), the
...
- The `country` label carries **19 distinct values, not the 16 countries the
  ...
  which sum to the 543 reported. `physiographic_synthesis.py` consolidates the
...
- Two intermediates from the working directory are deliberately absent.
  `interannual_persistence_DJF.parquet` was not regenerated when the KGE screen was tightened
